...
)
{
    ...
    if ($clusterDNS)
    {
        ...
        $PropertyBag = @{Namespace=$ns;ClusterDNS=$ClusterDNS;Deployment=$Deployment;ClusterVIP=$ClusterVIP;GatewayDnsFormat=$oldGatewayDnsFormat;AlternateGatewayDnsFormat=$GatewayDnsFormat;DirectAddresses=$DirectAddresses;GvDirectAddresses=$GvDirectAddresses;Notes=$Disclaimer;CheckForNewNodes=$checkForNewNodes;ClusterRegion=$region}
    }
