...
$runtime = if ($ENV:DotNetRunTime -ne $null) { $ENV:DotNetRunTime } else { 'net8.0' }
...
$appNamespace = 'Microsoft.Azure.Relay'
$testNamespace = 'Microsoft.Azure.Relay.UnitTests'

# Environment variables
$connectionStringVariableName = 'azure-relay-dotnet/ConnectionString'
$codeCovSecret = [Environment]::GetEnvironmentVariable('azure-relay-dotnet/CodeCovSecret')
$clientSecret = [Environment]::GetEnvironmentVariable('azure-relay-dotnet/ClientSecret')
$tenantId = [Environment]::GetEnvironmentVariable('azure-relay-dotnet/TenantId')
$appId = [Environment]::GetEnvironmentVariable('azure-relay-dotnet/AppId')
$canDeploy = ([bool]$clientSecret -and [bool]$tenantId -and [bool]$appId)
$skipCodeCoverage = if ([bool][Environment]::GetEnvironmentVariable('azure-relay-dotnet/SkipCodeCoverage')) { $true } else { $false }
...
function Build-Solution
{
    ...
    MSBuild.exe Microsoft.Azure.Relay.sln /t:restore /p:Configuration=$configuration /p:Platform=$platform /verbosity:minimal
    ...
    # Build solution
   MSBuild.exe Microsoft.Azure.Relay.sln /p:Configuration=$configuration /p:Platform=$platform /verbosity:minimal
...
{
    $resourceNamePrefix = 'relay-dotnet-av-'
    ...
    $location =
    if ([bool][Environment]::GetEnvironmentVariable('azure-relay-dotnet/location'))
    {
        [Environment]::GetEnvironmentVariable('azure-relay-dotnet/location')
    } else { 'westus' }
